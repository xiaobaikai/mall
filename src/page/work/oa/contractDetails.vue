...
export default {
  ...
  methods: {
          ...
          if (res.data.h.code != 200) {
            ...
            setTimeout(() => {
              ...
              window.location.href =
                "epipe://?&mark=contractDetails&_id=" +
                that.dataObj.contractId +
                "&data=" +
    ...
    go_fildDetails: function(url) {
      //查看图片详情
      let that = this;
      let obj = { index_num: 0, data: [url], type: 0 };
      window.location.href =
        "epipe://?&mark=imgdetail&url=" + JSON.stringify(obj);
    },
      ...
      .then(function(res) {
        ...
        for (let i = 0; i < that.dataObj.auditers.length; i++) {
          ...
          if(that.dataObj.auditers[i].accessory != null) {
            that.dataObj.auditers[i].accessory = that.accessoryFors(
              that.dataObj.auditers[i].accessory
            );
          }
        }
        ...
        if (
          that.dataObj.auditers[that.dataObj.auditers.length - 1].status == 1
        ) {
          // 已同意
          that.leaveType = "1";
          return;
...
.content {
    ...
    .styles {
        ...
        -webkit-box-shadow: 0 0 0.2rem rgba(255, 136, 0, 0.1);
        box-shadow: 0 0 0.2rem rgba(255, 136, 0, 0.1);
