<template>
    <section class="trip">
        ...
        <div class="content">
            ...
            <div v-for="(item,index) in dataObj.tripList" :key="index">
                <div class="tripCount" v-if="dataObj.tripList.length>1" >行程明细 {{index+1}}</div>
...
<script>
    ...
    export default {
        ...
        ...mapMutations(['change_man','approver_man']),
            refuse:function(){
                 this.$router.push({path:'/opinion',query:{id:this.dataObj.tripId,type:'trip',color:'#609df6'}})
            },
            ...
            deliverTo(){ //转交
                ...
                this.$router.push({path:'/imchoices',query:{id:this.dataObj.tripId,careOf:true,type:'trip',auditerIds:newApprStr,num:1}})
            },
            approveBack(){ //退回
                 this.$router.push({path:'/approveBack',query:{id:this.dataObj.tripId,type:'trip',color:'#609df6'}})
            },
            resubmit(){ //再次提交
                this.$router.replace({path:'/trip',query:{tripId:this.dataObj.tripId,resubmit:true}})
            },
            urge(){ //催办
                let that = this;
            ...
            consent:function(){ //同意
               ...
               this.$router.push({path:'/opinion',query:{id:this.dataObj.tripId,receiverIds:copyStr,auditerIds:apprStr,color:'#609df6',type:'trip',pageType:'consent'}})
            },
            moreBtn(){
                this.isShow = true;
		        this.$forceUpdate();
        ...
        mounted:function(){
            ...
            this.axios.get('/work/trip/info?tripId='+this.tripId).then(function(res){
                that.dataObj = res.data.b;
                let arr = [];
                that.accessory = that.accessoryFors(that.dataObj.accessory)
                that.title = that.dataObj.username+'的出差申请'
