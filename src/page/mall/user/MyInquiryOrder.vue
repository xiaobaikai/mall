  ...
  <div class="wrapper">
    ...
    <div class="order-list">
      <div v-for="(item,index) in orderList" :key="index"  @click="linkDetails(item)">
        <inquiry-order-item :obj="item" :imgPrefix="imgPrefix"  @childCall="handleChildCall"></inquiry-order-item>
      </div>
...
<script>
	...
	export default {
		...
		data(){
			return{
				menuList: [{title: "询价单",code: ""},{title: "卖家报价",code: 0},{title: "买家确认",code: 1}, {title: "已下单",code: 2},{title: "已取消",code: 4}],
				selected: 0,
				orderList: [],
				pageSize: 10,
				pageNo: 1,
		...
		methods:{
			...
			infiniteHandler($state){
				setTimeout(() =>{
					this.axios.post(this.baseURL.mall + '/m/my/myInquiryOrders' + this.Service.queryString({
						token: this.mallToken.getToken() || this.$route.query.token,
						pageSize: this.pageSize || "",
						pageNo: this.pageNo || "",
						sellerEnsure: this.orderState,
					})).then(res =>{
			...
			/*跳转询价单详情*/
			linkDetails(item){
				this.$router.push({path: '/InquiryOrderDetails', query:{inquiryId:item.inquiryId,imgPrefix:this.imgPrefix}});
			}
		}
	}
</script>
